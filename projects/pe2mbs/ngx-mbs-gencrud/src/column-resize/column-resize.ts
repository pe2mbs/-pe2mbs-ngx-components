...
export abstract class ColumnResize implements AfterViewInit, OnDestroy 
{
    ...
    public getUniqueCssClass(): string 
    {
        return ( `cdk-column-resize-${ this.selectorId }` );
    }

    /** Called when a column in the table is resized. Applies a css class to the table element. */
    public setResized(): void 
    {
    ...
    private _listenForRowHoverEvents(): void
    {
        this.ngZone.runOutsideAngular(() => {
            ...
            fromEvent<MouseEvent>(element, 'mouseover').pipe(
                ...
                takeUntil( this.destroyed ) ).subscribe( this.eventDispatcher.headerCellHovered );
            fromEvent<MouseEvent>(element, 'mouseleave').pipe( filter( event => 
                !!event.relatedTarget && !(event.relatedTarget as Element).matches( RESIZE_OVERLAY_SELECTOR ) ), 
                mapTo( null ), takeUntil( this.destroyed ) ).subscribe( this.eventDispatcher.headerCellHovered );
        } );
        return;
    }

    private _listenForResizeActivity(): void 
    {
        merge( this.eventDispatcher.overlayHandleActiveForCell.pipe( mapTo( undefined ) ),
    ...
    private _listenForHoverActivity(): void 
    {
        this.eventDispatcher.headerRowHoveredOrActiveDistinct.pipe( startWith( null ), pairwise(), takeUntil( this.destroyed ) )
