...
export abstract class ResizeOverlayHandle implements AfterViewInit, OnDestroy 
{
    ...
    private _listenForMouseEvents(): void 
    {
        this.ngZone.runOutsideAngular( () => {
            ...
            fromEvent<MouseEvent>(this.elementRef.nativeElement!, 'mouseleave').pipe( map( event =>
                    event.relatedTarget && closest(event.relatedTarget as Element, HEADER_CELL_SELECTOR),
            ), takeUntil(this.destroyed), ).subscribe( cell => this.eventDispatcher.headerCellHovered.next( cell ) );

            fromEvent<MouseEvent>(this.elementRef.nativeElement!, 'mousedown').pipe(takeUntil(this.destroyed))
    ...
    private _dragStarted( mousedownEvent: MouseEvent ): void 
    {
        ...
        mousemove.pipe( map(({ screenX }) => screenX),
            startWith(startX),
            ...
            computedNewSize = Math.min( Math.max(computedNewSize, this.resizeRef.minWidthPx, 0),
